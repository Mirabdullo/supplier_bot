...
export async function sendRejectedOrders(ctx: Context, pageIndex: number, compId: string) {
    try {
        ...
        if (products) {
            ...
            if (pageProducts.length > 0) {
                let message = `Результаты <b>непринятые</b>: ${PAGE_SIZE * pageIndex + 1}-${PAGE_SIZE * pageIndex + PAGE_SIZE} из ${
                    products.length
                }\n\n`;
                let i = 0;
                let keyboardArray = [];
                ...
                ctx.reply("Отмененные заказы пока нет!", {
                    parse_mode: "HTML",
                    reply_markup: {
